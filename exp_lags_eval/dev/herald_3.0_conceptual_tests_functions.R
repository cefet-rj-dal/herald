...
source(paste0(getwd(),"/dev/nexus/nexus.R"))
...
herald_nexus <- function(model,data, full=FALSE, png_folder="dev/plots/",title=TRUE,exp_descr="", verbose=TRUE){
  ...
  online_detector$plot <- list()
  online_detection_hist <- NULL
  while (!is.null(online_detector$datasource)) {
    
    online_detector <- detect(online_detector)
    
    online_detection_hist <- c(online_detection_hist, which(as.logical(online_detector$detection$event)))
    #if(verbose) print(paste0("Batch ",bt))

    #event happened
    if(sum(data$event[online_detector$detection$idx]) > evt_count & !event_happened){
      if(verbose) print("Event happened!")
      event_happened <- TRUE
      event_idx <- tail(which(data$event[online_detector$detection$idx]),1)
      evt_idx <- as.character(event_idx)
    ...
    if(any(which(as.logical(online_detector$detection$event))>event_idx) & event_happened) {
      ...
      online_detector$detection_lag[[evt_idx]] <- list()
      online_detector$detection_lag[[evt_idx]]$batch <- bt-bt_event_happened
      det.index <- which(as.logical(online_detector$detection$event))
      online_detector$detection_lag[[evt_idx]]$obs <- min(det.index[det.index>event_idx])-event_idx
      online_detector$detection_lag[[evt_idx]]$time <- round(as.numeric(difftime(time1 = Sys.time(), time2 = time_event_happened, units = "secs")), 3)
      
      
      #for plotting (move out of this if for gif generation)
      if(title) {
        det_lag <- paste0("Detection lag: ",online_detector$detection_lag[[evt_idx]]$batch," batches, ",
                          online_detector$detection_lag[[evt_idx]]$obs," observations, ",
                          online_detector$detection_lag[[evt_idx]]$time," seconds")
      ...
      online_detector$plot[[evt_idx]] <- grf
  ...
  if(length(online_detector$detection_lag)==0){
    for(evt in as.character(which(data$event))) online_detector$detection_lag[[evt]] <- list()
  }
  
  online_detector$detection$event[online_detection_hist] <- 1 
  
  #plotting final results
  if(title) plot_title <- paste0("Settings: ",exp_descr,"\n")
  ...
  require(ggtext)
  if(title) grf <- grf + ggtitle(plot_title) + theme(plot.title = element_textbox_simple())
  online_detector$plot[["final"]] <- grf
  
  online_detector$evaluation <- list()
  if(any(as.logical(online_detector$detection$event))){
    evaluation_soft <- evaluate(har_eval_soft(sw_size=15), as.logical(online_detector$detection$event), data$event)
    evaluation_hard <- evaluate(online_detector$detector, as.logical(online_detector$detection$event), data$event)
    online_detector$evaluation[["hard"]] <- evaluation_hard
    online_detector$evaluation[["soft"]] <- evaluation_soft
  }
...
run_exp <- function(data,id,pred=TRUE,lag_pred=0, mdl_train=30, online_step=1, pred_ic=FALSE, pred_par=NULL, pred_recursive=FALSE, 
  ...
  event_idx <- names(herald_detection$detection_lag)
  if(length(herald_detection$detection_lag[[1]])==0){
    detlag_batch <- detlag_obs <- detlag_time <- NA
  }
  else{
    detlag_batch <- sapply(event_idx,function(evt) herald_detection$detection_lag[[evt]]$batch)
    detlag_obs <- sapply(event_idx,function(evt) herald_detection$detection_lag[[evt]]$obs)
    detlag_time <- sapply(event_idx,function(evt) herald_detection$detection_lag[[evt]]$time)
  }
  

  if(length(herald_detection$evaluation)>0){
    herald_detection$evaluation[["hard"]]$confMatrix <- herald_detection$evaluation[["soft"]]$confMatrix <- NULL
    eval_hard <- do.call(cbind,herald_detection$evaluation[["hard"]])
    colnames(eval_hard) <- paste(colnames(eval_hard),"hard",sep="_")
    eval_soft <- do.call(cbind,herald_detection$evaluation[["soft"]])
    colnames(eval_soft) <- paste(colnames(eval_soft),"soft",sep="_")
  }
  else {
    metric_names <- list("TP","FP","FN","TN","accuracy",
                         "sensitivity","specificity","prevalence", "PPV", "NPV",
                         "detection_rate","detection_prevalence",
                         "balanced_accuracy","precision","recall","F1")
    eval_hard <- eval_soft <- as.data.frame(t(rep(NA,length(metric_names))))
    colnames(eval_hard) <- paste(metric_names,"hard",sep="_")
    metric_names <- list("TPs","FPs","FNs","TNs","accuracy",
                         "sensitivity","specificity","prevalence", "PPV", "NPV",
                         "detection_rate","detection_prevalence",
                         "balanced_accuracy","precision","recall","F1")
    colnames(eval_soft) <- paste(metric_names,"soft",sep="_")
  }
  
  #Writing results into data.frame
  res_path_lags <- sub('.rds$', '_lags.rds', res_path)
  res_path_eval <- sub('.rds$', '_eval.rds', res_path)
  if(file.exists(res_path_lags)) {
    results_lags <- readRDS(res_path_lags)
    results_eval <- readRDS(res_path_eval)
  }
  else {
    results_lags <- data.frame()
    results_eval <- data.frame()
  }
  
  exp_res_lags <- do.call(cbind,list(id=id,pred=pred,lag=lag_pred,mdl=mdl_train,ic=pred_ic,par=ifelse(pred_ic,pred_par[[1]],NA),
                                recursive=pred_recursive,mem=mem_dist,conf=conf,
                                event_idx=event_idx, detlag_batch=detlag_batch,detlag_obs=detlag_obs,detlag_time=detlag_time))
  
  exp_res_eval <- cbind(id=id,pred=pred,lag=lag_pred,mdl=mdl_train,ic=pred_ic,par=ifelse(pred_ic,pred_par[[1]],NA),
                        recursive=pred_recursive,mem=mem_dist,conf=conf,eval_hard,eval_soft)
  
  results_lags <- rbind(results_lags,exp_res_lags)
  results_eval <- rbind(results_eval,exp_res_eval)
  saveRDS(results_lags,file=res_path_lags)
  saveRDS(results_eval,file=res_path_eval)
  
  return(list(detection=herald_detection,results=list(lags=results_lags,eval=results_eval)))
}
...
experiments <- function(data,parameters,verbose=TRUE){
  ...
  res_path_eval <- sub('.rds$', '_eval.rds', res_path)
  if(file.exists(res_path_lags)) {
    file.remove(res_path_lags) #Reset memory of test results
    file.remove(res_path_eval) #Reset memory of test results
  }
...
trend_sym <- function(n=100,ts.var=0.1,seed=12345,plot=TRUE,
                      trend_function=c("linear","log","sqrt","poly2","poly3","exp","sigmoid"),flip=FALSE) {
  ...
  if(flip){
    time_series <- c(time_series,rev(time_series))
    data <- data.frame(serie=time_series,event=FALSE)
    data[seq(n,3*n,by=n), "event"] <- TRUE
  }
  else{
    data <- data.frame(serie=time_series,event=FALSE)
    data[n, "event"] <- TRUE
